...
EventLoopThreadPool::EventLoopThreadPool(EventLoop* base_loop, uint32_t thread_number)
    ...
    DEVPP_LOG_TRACE << "thread_num=" << thread_num() << " base loop=" << base_loop_;
}

EventLoopThreadPool::~EventLoopThreadPool() {
    DEVPP_LOG_TRACE << "thread_num=" << thread_num();
    Join();
    threads_.clear();
}
...
bool EventLoopThreadPool::Start(bool wait_thread_started) {
    ...
    DEVPP_LOG_TRACE << "thread_num=" << thread_num() << " base loop=" << base_loop_ << " wait_thread_started=" << wait_thread_started;
    ...
    for (uint32_t i = 0; i < thread_num_; ++i) {
        auto prefn = [this, started_count]() {
            DEVPP_LOG_TRACE << "a working thread started tid=" << std::this_thread::get_id();
            this->OnThreadStarted(started_count->fetch_add(1) + 1);
            return EventLoopThread::kOK;
        };

        auto postfn = [this, exited_count]() {
            DEVPP_LOG_TRACE << "a working thread exiting, tid=" << std::this_thread::get_id();
        ...
        if (!t->Start(wait_thread_started, prefn, postfn)) {
            //FIXME error process
            EVPP_LOG_ERROR << "start thread failed!";
            return false;
        }
...
void EventLoopThreadPool::Stop(bool wait_thread_exit) {
    DEVPP_LOG_TRACE << "wait_thread_exit=" << wait_thread_exit;
...
void EventLoopThreadPool::Stop(bool wait_thread_exit, DoneCallback fn) {
    ...
    if (thread_num_ == 0) {
        ...
        if (fn) {
            DEVPP_LOG_TRACE << "calling stopped callback";
            fn();
        }
        return;
    }
    ...
    DEVPP_LOG_TRACE << "before promise wait";
    if (thread_num_ > 0 && wait_thread_exit) {
        while (!is_stopped_fn()) {
            usleep(1);
        }
    }
    DEVPP_LOG_TRACE << "after promise wait";
...
void EventLoopThreadPool::OnThreadStarted(uint32_t count) {
    DEVPP_LOG_TRACE << "tid=" << std::this_thread::get_id() << " count=" << count << " started.";
    if (count == thread_num_) {
        DEVPP_LOG_TRACE << "thread pool totally started.";
        status_.store(kRunning);
    }
}

void EventLoopThreadPool::OnThreadExited(uint32_t count) {
    DEVPP_LOG_TRACE << "tid=" << std::this_thread::get_id() << " count=" << count << " exited.";
    if (count == thread_num_) {
        status_.store(kStopped);
        DEVPP_LOG_TRACE << "this is the last thread stopped. Thread pool totally exited.";
        if (stopped_cb_) {
            stopped_cb_();
