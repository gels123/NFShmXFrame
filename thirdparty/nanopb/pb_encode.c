...
#define APPEND_ERRMSG(stream, field, format, ... ) \
    ...
    { \
        snprintf( \
            ...
            , "\n[file:(%s:%d(%s)), stream(bytes_written:%u max_size:%u) " format "]" \
            , __FILE__, __LINE__, __FUNCTION__ \
            , (uint32_t)(stream->bytes_written) \
            , (uint32_t)(stream->max_size) \
            , ##__VA_ARGS__ \
        ); \
...
bool checkreturn pb_write(pb_ostream_t *stream, const pb_byte_t *buf, size_t count)
{
    if (stream->callback != NULL)
    {
        ...
        {
            APPEND_ERRMSG(stream, NULL, "(bytes_written + count:%u) must <= max_size", (uint32_t)count);
                         ...
                         const void *pData, size_t count, pb_encoder_t func)
{
    ...
    {
        APPEND_ERRMSG(stream, field, "array count:%u exceeded, must <= array_size", (uint32_t)count);
    ...
    const pb_field_t *field, const void *pData)
{
    ...
    switch (PB_HTYPE(field->type))
    {
        case PB_HTYPE_REQUIRED:
            ...
            if (!pb_encode_tag_for_field(stream, field))
            {
                APPEND_ERRMSG(stream, field, "pb_encode_tag_for_field ret fail");
                return false;
            }

            if (!func(stream, field, pData))
            {
                APPEND_ERRMSG(stream, field, "fun ret fail");
                return false;
            }
            break;
        ...
        case PB_HTYPE_REPEATED: {
            ...
            {
                APPEND_ERRMSG(stream, field, "encode_array ret fail, count:%u", count);
                return false;
            }
            break;
        }
        ...
        case PB_HTYPE_ONEOF:
            if (*(const pb_size_t*)pSize == field->tag)
            {
                ...
                if (!func(stream, field, pData))
                {
                    APPEND_ERRMSG(stream, field, "func ret fail");
                    return false;
                }
            }
            break;
        ...
        default:
        {
            APPEND_ERRMSG(stream, field, "invalid field type");
    ...
    const pb_field_t *field, const void *pData)
{
    switch (PB_ATYPE(field->type))
    {
        ...
        default:
        {
            APPEND_ERRMSG(stream, NULL, "invalid field type");
            PB_RETURN_ERROR(stream, "invalid field type");
        }
    }
}
...
bool checkreturn pb_encode(pb_ostream_t *stream, const pb_field_t fields[], const void *src_struct)
{
    ...
    do {
        ...
        else
        {
            ...
            {
                APPEND_ERRMSG(stream, iter.pos, "encode_field fail");
                return false;
            }
        }
    } while (pb_field_iter_next(&iter));
...
bool checkreturn pb_encode_submessage(pb_ostream_t *stream, const pb_field_t fields[], const void *src_struct)
{
    ...
    if (!pb_encode(&substream, fields, src_struct))
    {
        ...
        APPEND_ERRMSG(stream, NULL, "pb_encode fail, substream.sz_errmsg:%s", substream.sz_errmsg);
#endif
        return false;
    }
    ...
    {
        APPEND_ERRMSG(stream, NULL, "stream full, bytes_written + size:%u must <= max_size", (uint32_t)size);
        PB_RETURN_ERROR(stream, "stream full");
    }
...
#ifndef PB_NO_ERRMSG
    if (!status) // 子消息编码失败才记录日志 编码正常不需要记录错误日志
    {
        stream->errmsg = substream.errmsg;
        APPEND_ERRMSG(stream, NULL, "pb_encode fail, substream.sz_errmsg:%s", substream.sz_errmsg);
    }
...
static bool checkreturn pb_enc_bytes(pb_ostream_t *stream, const pb_field_t *field, const void *src)
{
    ...
    {
        APPEND_ERRMSG(stream, field, "byte size:%u exceeded, must <= data_size", bytes->size);
