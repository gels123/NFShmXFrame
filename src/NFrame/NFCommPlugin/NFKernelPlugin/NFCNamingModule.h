...
public:
    ...
    /// @param urls 地址列表
    /// @return 0成功，其他失败，错误码意义见@ref ZookeeperErrorCode
    virtual int32_t GetExternalServerIpsByName(NF_SERVER_TYPE eServerType, NF_SERVER_TYPE destServerType, std::vector<std::string>& busUrl) override;
    ...
    /// @param urls 地址列表
    /// @return 0成功，其他失败，错误码意义见@ref ZookeeperErrorCode
    virtual int32_t GetExternalServerPortsByName(NF_SERVER_TYPE eServerType, NF_SERVER_TYPE destServerType, std::vector<std::string>& busUrl) override;
...
public:
    ...
    /// @return 0成功
    virtual int32_t SetCache(NF_SERVER_TYPE eServerType, bool use_cache = true, int32_t refresh_time_ms = 300000, int32_t invaild_time_ms = 1800000) override;
public:
    // 实现Naming接口
