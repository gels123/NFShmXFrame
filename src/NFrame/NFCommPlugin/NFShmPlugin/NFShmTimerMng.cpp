...
bool NFShmTimerSlot::OnTick(NFShmTimerMng* pTimerManager, int64_t tick, list<NFShmTimer*>& timeoutList, uint32_t seq, NFTimerIdData* allIdData)
{
    ...
    while (tmpData->m_nextIndex != -1)
    {
        ...
        NFShmTimer* pTimer = pTimerManager->GetTimer(tmpData->m_objId);
...
NFTimerIdData* NFShmTimerSlot::UnBindListTimer(NFShmTimerMng* pTimerManager, NFShmTimer* timer, const NFTimerIdData* tmpData, NFTimerIdData* allIdData)
{
    if (!tmpData || !allIdData)
        return nullptr;

    NFTimerIdData* nextData = nullptr;
    NFTimerIdData* prefData = nullptr;
...
bool NFShmTimerSlot::DeleteTimer(NFShmTimerMng* pTimerManager, NFShmTimer* timer, NFTimerIdData* allIdData)
{
    ...
    NFTimerIdData* tmpData = &allIdData[timer->GetListIndex()];
...
int NFShmTimerMng::ResumeInit()
{
    ...
    while (pTimer)
    {
        ...
        pTimer = dynamic_cast<NFShmTimer*>(FindModule<NFIMemMngModule>()->GetNextObj(EOT_TYPE_TIMER_OBJ, pTimer));
    }
    return 0;
}
...
void NFShmTimerMng::OnTick(int64_t tick)
{
    int tickCount = 0;
    ...
    while (true)
    {
        ...
        for (it = lRetsetTimer.begin(); it != lRetsetTimer.end(); ++it)
        {
            ...
            if (!(*it)->IsWaitDelete())
            {
                ...
                {
                    // 如果不是一次性的定时器，就又加入槽
                    if ((*it)->GetType() != NFShmTimer::ONCE_TIMER &&
...
int NFShmTimerMng::AddTimer(NFShmTimer* timer, int64_t tick, bool isNewTimer)
{
    if (!timer)
        return -1;
...
int NFShmTimerMng::AddTimer(NFShmTimer* timer, int slot)
{
    ...
    NFTimerIdData* idData = GetFreeTimerIdData();
    if (!idData)
    {
...
bool NFShmTimerMng::CheckFull() const
{
...
void NFShmTimerMng::ReleaseTimerIdData(int index)
{
    ...
    NFTimerIdData* tmpBlock = &m_timerIdData[index];
...
NFTimerIdData* NFShmTimerMng::GetFreeTimerIdData()
{
    if (CheckFull())
    {
...
bool NFShmTimerMng::AttachTimer(NFShmTimer* timer, int64_t tick, bool isNewTimer)
{
    ...
    {
        if (timer->GetType() == NFShmTimer::MONTH_LOOP_TIMER)
        {
            // 间隔月的天数不同，需要额外算间隔时间
...
bool NFShmTimerMng::SetDistanceTime(NFShmTimer* stime, int hour, int minutes, int second, int microSec, int interval, int callCount)
{
...
bool NFShmTimerMng::SetDayTime(NFShmTimer* stime, uint64_t timestamp, int interval, int callCount)
{
    int64_t diffTime = 0;
    int64_t nowTime = NF_ADJUST_TIMENOW_MS();
...
bool NFShmTimerMng::SetDayTime(NFShmTimer* stime, int hour, int minutes, int second, int interval, int callCount)
{
    int64_t diftime = 0;
...
bool NFShmTimerMng::SetWeekTime(NFShmTimer* stime, int weekDay, int hour, int minutes, int second, int callCount)
{
    int64_t diftime = 0;
    time_t t = NF_ADJUST_TIMENOW();
    int nowWeek = 0, nowHour = 0, nowMinute = 0, nowSecond = 0;
...
bool NFShmTimerMng::SetMonthTime(NFShmTimer* stime, int day, int hour, int minutes, int second, int callCount)
{
    int64_t diffTime = 0;
    time_t t = NF_ADJUST_TIMENOW();
    int nmonth = 0;
...
int NFShmTimerMng::ClearShmObjTimer(NFShmTimer* pTimer)
{
    CHECK_NULL(0, pTimer);
...
int NFShmTimerMng::ClearAllTimer(const NFObject* pObj)
{
    CHECK_NULL(0, pObj);
...
int NFShmTimerMng::ClearAllTimer(const NFObject* pObj, const NFRawObject* pRawShmObj)
{
    CHECK_NULL(0, pObj);
...
int NFShmTimerMng::AddShmObjTimer(const NFObject* pObj, NFShmTimer* pTimer)
{
    auto iter = m_shmObjTimer.find(pObj->GetGlobalId());
    if (iter == m_shmObjTimer.end())
    {
        iter = m_shmObjTimer.emplace_hint(m_shmObjTimer.end(), MakePair(pObj->GetGlobalId(), NFNodeObjList<NFShmTimer>()));
        if (iter == m_shmObjTimer.end())
        {
...
int NFShmTimerMng::SetTimer(const NFObject* pObj, int hour, int minutes, int second, int microSec, const NFRawObject* pRawShmObj/* = NULL*/)
{
    NFShmTimer* newTimer = NFShmTimer::CreateObj();
    if (!newTimer)
    {
    ...
    newTimer->SetType(NFShmTimer::ONCE_TIMER);
...
int NFShmTimerMng::SetCalender(const NFObject* pObj, uint64_t timestamp, const NFRawObject* pRawShmObj/* = NULL*/)
{
...
int
NFShmTimerMng::SetCalender(const NFObject* pObj, int hour, int minutes, int second, const NFRawObject* pRawShmObj/* = NULL*/)
{
...
int NFShmTimerMng::SetTimer(const NFObject* pObj, int interval, int callCount, int hour, int minutes, int second, int microSec, const NFRawObject* pRawShmObj/* = NULL*/)
{
    ...
    newTimer->SetTimerShmObj(pObj);
    newTimer->SetTimerRawShmObj(pRawShmObj);
    newTimer->SetType(NFShmTimer::LOOP_TIMER);
    if (callCount <= 0)
    {
...
int NFShmTimerMng::SetDayTime(const NFObject* pObj, int callCount, int hour, int minutes, int second, int microSec, const NFRawObject* pRawShmObj/* = NULL*/)
{
    NFShmTimer* newTimer = NFShmTimer::CreateObj();
    if (!newTimer)
    {
...
int NFShmTimerMng::SetDayCalender(const NFObject* pObj, int callCount, int hour, int minutes, int second, const NFRawObject* pRawShmObj/* = NULL*/)
{
    CHECK_EXPR(hour >= 0 && hour <= 23, INVALID_ID, "Create timer hour err");
    CHECK_EXPR(minutes >= 0 && minutes <= 59, INVALID_ID, "Create timer minute err");
    CHECK_EXPR(second >= 0 && second <= 59, INVALID_ID, "Create timer second err");
...
int NFShmTimerMng::SetWeekTime(const NFObject* pObj, int callCount, int hour, int minutes, int second, int microSec, const NFRawObject* pRawShmObj/* = NULL*/)
{
    NFShmTimer* newTimer = NFShmTimer::CreateObj();
    if (!newTimer)
    {
...
int NFShmTimerMng::SetWeekCalender(const NFObject* pObj, int callCount, int weekDay, int hour, int minutes, int second, const NFRawObject* pRawShmObj/* = NULL*/)
{
...
int NFShmTimerMng::SetMonthTime(const NFObject* pObj, int callCount, int hour, int minutes, int second, int microSec, const NFRawObject* pRawShmObj/* = NULL*/)
{
    NFShmTimer* newTimer = NFShmTimer::CreateObj();
    if (!newTimer)
    {
    ...
    newTimer->SetTimerShmObj(pObj);
    newTimer->SetTimerRawShmObj(pRawShmObj);
    newTimer->SetType(NFShmTimer::MONTH_LOOP_TIMER);
...
int NFShmTimerMng::SetMonthCalender(const NFObject* pObj, int callCount, int day, int hour, int minutes, int second, const NFRawObject* pRawShmObj/* = NULL*/)
{
