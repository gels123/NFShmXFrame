...
int NFCAppInited::RegisterAppTask(NF_SERVER_TYPE eServerType, uint32_t taskType, const std::string& desc, uint32_t taskGroup)
{
    ...
    {
        for (int i = 1; i < static_cast<int>(m_serverTaskGroup.size()); i++)
        {
...
int NFCAppInited::CheckTaskFinished()
{
    ...
    for (int i = 0; i < static_cast<int>(m_taskGroupFlag.size()); i++)
    {
        if (!m_taskGroupFlag[i])
        {
            ...
            for (int j = 1; j < static_cast<int>(m_serverTaskGroup.size()); j++)
            {
                if (m_serverTaskGroup[j].m_taskGroupList[i].m_finishAllFlag == false)
                {
                    ...
                    if (m_serverTaskGroup[j].m_taskGroupList[i].m_finishAllFlag)
                    {
                        ...
                        FindModule<NFIEventModule>()->FireExecute(static_cast<NF_SERVER_TYPE>(j), NFrame::NF_EVENT_SERVER_TASK_GROUP_FINISH, NFrame::NF_EVENT_SERVER_TYPE, i, event);
                    }
                }
            }
            ...
            {
                NFrame::NFEventNoneData event;
                FindModule<NFIEventModule>()->FireExecute(NF_ST_NONE, NFrame::NF_EVENT_SERVER_TASK_GROUP_FINISH, NFrame::NF_EVENT_SERVER_TYPE, i, event);
    ...
    for (int i = 1; i < static_cast<int>(m_serverTaskGroup.size()); i++)
    {
        if (!m_serverTaskGroup[i].m_isInited)
        {
            ...
            if (flag)
            {
                ...
                NFrame::NFEventNoneData event;
                FindModule<NFIEventModule>()->FireExecute(static_cast<NF_SERVER_TYPE>(i), NFrame::NF_EVENT_SERVER_APP_FINISH_INITED, NFrame::NF_EVENT_SERVER_TYPE, 0, event);

                if (m_pObjPluginManager->IsLoadAllServer())
                {
                    NFLogInfo(NF_LOG_DEFAULT, 0, "Server:{} Finish All Task, App Inited Success..............", GetServerName(static_cast<NF_SERVER_TYPE>(i)));
                }
            }
        }
    }
