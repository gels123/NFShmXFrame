...
bool PbToJsonConverter::Convert(const google::protobuf::Message& message, Handler& handler) {
    ...
    for (int i = 0; i < field_count; ++i) {
        ...
        if (_option.enable_protobuf_map && NFJson2PB::IsProtobufMap(field)) {
    ...
    for (size_t i = 0; i < map_fields.size(); ++i) {
        ...
        const google::protobuf::FieldDescriptor* key_desc =
                map_desc->message_type()->field(NFJson2PB::KEY_INDEX);
        const google::protobuf::FieldDescriptor* value_desc =
                map_desc->message_type()->field(NFJson2PB::VALUE_INDEX);
        ...
        if (field->is_repeated()) {
            ...
            for (int index = 0; index < field_size; ++index) {
                ...
                if (field->type() == google::protobuf::FieldDescriptor::TYPE_BYTES
                    ...
                    std::string value_decoded = NFBase64::Encode(value);
...
bool ProtoMessageToJson(const google::protobuf::Message& message,
    ...
    if (NFJson2PB::ProtoMessageToJsonStream(message, options, buffer, error)) {
...
bool ProtoMessageToJson(const google::protobuf::Message& message,
	...
	NFJson2PB::NFZeroCopyStreamWriter wrapper(stream);
    return NFJson2PB::ProtoMessageToJsonStream(message, options, wrapper, error);
}
