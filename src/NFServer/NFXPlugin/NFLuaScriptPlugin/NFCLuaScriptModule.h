...
class NFLuaEventObj : public NFBaseObj
{
public:
	/**
	 *@brief 构造函数
	 */
	NFLuaEventObj(NFIPluginManager* p, NFCLuaScriptModule* pModule) : NFBaseObj(p),
																	  m_pModule(pModule)
	{
	}
...
public:
	 ...
	 * @return
	 */
	virtual bool RegisterClientMessage(NF_SERVER_TYPE eType, uint32_t nMsgID, const std::string& strLuaFunc, bool createCo = false);
	 ...
	 * @return
	 */
	virtual bool RegisterServerMessage(NF_SERVER_TYPE eType, uint32_t nMsgID, const std::string& strLuaFunc, bool createCo = false);
...
public:
	 ...
	 * 添加连接事件，掉线事件的处理函数
	 * */
	virtual bool AddEventCallBack(NF_SERVER_TYPE eType, uint64_t linkId, const std::string& strLuaFunc, bool createCo = false);

	/*
	 * 未没有注册过的消息，添加一个统一处理的回调函数
	 * */
	virtual bool AddOtherCallBack(NF_SERVER_TYPE eType, uint64_t linkId, const std::string& strLuaFunc, bool createCo);

	/*
	* 对所有的消息添加一个统一的回调， 同过判断返回, 0表示将处理这个消息，!=0将不处理这个消息
	* */
	virtual bool AddAllMsgCallBack(NF_SERVER_TYPE eType, const std::string& strLuaFunc, bool createCo);
public:
	/*
		处理客户端链接事件和未注册消息
	*/
...
public:
	virtual int SendMsgToMasterServer(NF_SERVER_TYPE eSendTyp, uint32_t nMsgId, const std::string& xData, uint64_t nParam1 = 0,
									  uint64_t nParam2 = 0);
public:
	////////////////////////////send proxy msg to other serer//////////////////////////////////
	virtual int SendProxyMsgByBusId(NF_SERVER_TYPE eType, uint32_t nDstId, uint32_t nModuleId, uint32_t nMsgId, const std::string& xData,
...
public:
	...
	virtual int SendMsgToWorldServer(NF_SERVER_TYPE eType, uint32_t nMsgId, const std::string& xData, uint64_t nParam1 = 0,
									 uint64_t nParam2 = 0);

	virtual int SendTransToWorldServer(NF_SERVER_TYPE eType, uint32_t nMsgId, const std::string& xData, uint32_t req_trans_id = 0,
									   uint32_t rsp_trans_id = 0);

	///////////////////////////other server send msg to game msg/////////////////////////////
	virtual int SendMsgToGameServer(NF_SERVER_TYPE eType, uint32_t nDstId, uint32_t nMsgId, const std::string& xData,
									uint64_t nParam1 = 0, uint64_t nParam2 = 0);

	virtual int SendTransToGameServer(NF_SERVER_TYPE eType, uint32_t nDstId, uint32_t nMsgId, const std::string& xData, uint32_t req_trans_id = 0,
									  uint32_t rsp_trans_id = 0);

	///////////////////////////other server send msg to logic server/////////////////////////////
	virtual int SendMsgToLogicServer(NF_SERVER_TYPE eType, uint32_t nDstId, uint32_t nMsgId, const std::string& xData,
									 uint64_t nParam1 = 0, uint64_t nParam2 = 0);

	virtual int SendTransToLogicServer(NF_SERVER_TYPE eType, uint32_t nDstId, uint32_t nMsgId, const std::string& xData, uint32_t req_trans_id = 0,
									   uint32_t rsp_trans_id = 0);

	///////////////////////////other server send msg to sns server/////////////////////////////
	virtual int SendMsgToSnsServer(NF_SERVER_TYPE eType, uint32_t nMsgId, const std::string& xData, uint64_t nParam1 = 0,
								   uint64_t nParam2 = 0);

	virtual int SendTransToSnsServer(NF_SERVER_TYPE eType, uint32_t nMsgId, const std::string& xData, uint32_t req_trans_id = 0,
									 uint32_t rsp_trans_id = 0);
public:
	virtual bool AddRpcService(NF_SERVER_TYPE serverType, uint32_t nMsgId, const std::string& reqType, const std::string& rspType,
							   const std::string& strLuaFunc, bool createCo = false);

	virtual std::tuple<std::string, int> GetRpcService(NF_SERVER_TYPE serverType, NF_SERVER_TYPE dstServerType, uint32_t dstBusId, uint32_t msgId, const std::string& reqType,
...
public:
	///////////////////////////////////db rpc///////////////////////////////////////////////////
	virtual std::tuple<std::string, int> GetRpcSelectObjService(NF_SERVER_TYPE eType, uint64_t mod_key, const std::string& reqType,
	...
	virtual int GetRpcInsertObjService(NF_SERVER_TYPE eType, uint64_t mod_key,  const std::string& reqType, const std::string& request, uint32_t dstBusId = 0, const std::string& dbname = "");
public:
...
public:
	virtual std::vector<NF_SHARE_PTR<NFServerData>> GetServerByServerType(NF_SERVER_TYPE eSendType, NF_SERVER_TYPE serverTypes);

	virtual NF_SHARE_PTR<NFServerData> GetFirstServerByServerType(NF_SERVER_TYPE eSendType, NF_SERVER_TYPE serverTypes);

	virtual NF_SHARE_PTR<NFServerData> GetFirstServerByCross(NF_SERVER_TYPE eSendType, NF_SERVER_TYPE serverTypes, bool crossServer);

	virtual NF_SHARE_PTR<NFServerData> GetRandomServerByServerType(NF_SERVER_TYPE eSendType, NF_SERVER_TYPE serverTypes);

	virtual NF_SHARE_PTR<NFServerData> GetRandomServerByCross(NF_SERVER_TYPE eSendType, NF_SERVER_TYPE serverTypes, bool crossServer);

	virtual NF_SHARE_PTR<NFServerData> GetSuitServerByInt(NF_SERVER_TYPE eSendType, NF_SERVER_TYPE serverTypes, uint64_t value);

	virtual NF_SHARE_PTR<NFServerData> GetSuitServerByIntCross(NF_SERVER_TYPE eSendType, NF_SERVER_TYPE serverTypes, uint64_t value, bool crossServer);

	virtual NF_SHARE_PTR<NFServerData> GetSuitServerByStr(NF_SERVER_TYPE eSendType, NF_SERVER_TYPE serverTypes, const std::string &value);

	virtual NF_SHARE_PTR<NFServerData> GetSuitServerByStrCross(NF_SERVER_TYPE eSendType, NF_SERVER_TYPE serverTypes, const std::string &value, bool crossServer);
public:
