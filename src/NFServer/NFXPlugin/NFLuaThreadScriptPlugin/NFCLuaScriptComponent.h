		...
		if (pLuaTask)
		{
			if (pLuaTask->m_pComponent)
			{
				if (pLuaTask->m_pComponent->IsDeadCycle())
				{
					...
					NFLogError(NF_LOG_DEFAULT, 0, "the actor will start new lua state in the actor:{} actorId:{}, the system will kill the lua func", pLuaTask->m_pComponent->GetComponentName(), pLuaTask->m_pComponent->GetActorId());
	...
	*/
	virtual void HandleTaskDeadCycle(const std::string& taskName, uint64_t useTime)
	{
		if (!m_pPluginManager->IsLoadAllServer() && useTime > 120000)
		{
			...
			SetDeadCycle();
			NFLogError(NF_LOG_DEFAULT, 0, "asyc task:{} use time:{}, DEAD CYCLE in the actor:{} actorId:{}, the system will kill the lua func", taskName, useTime, GetComponentName(), GetActorId());
	...
	*/
	virtual void HandleTaskTimeOut(const std::string& taskName, uint64_t useTime)
	{
		if (!m_pPluginManager->IsLoadAllServer() && useTime > 5000)
		{
			...
			NFLogWarning(NF_LOG_DEFAULT, 0, "asyc task:{} use time:{}, may be dead cycle in the actor:{} actorId:{}", taskName, useTime, GetComponentName(), GetActorId());
		}
	}
public:
