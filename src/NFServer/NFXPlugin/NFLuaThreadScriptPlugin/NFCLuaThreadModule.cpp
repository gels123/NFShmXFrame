...
void NFCLuaThreadModule::HandleLuaTcpMsg()
{
	...
	if (ret)
	{
		if (!listTask.empty())
		{
			...
			{
				NFLogError(NF_LOG_DEFAULT, 0, "HandleLuaTcpMsg: send tcp msg:{} use time:{} ms", listTask.size(), useTime / 1000);
			}
		}
	}
}
...
void NFCLuaThreadModule::SendHttpServerMsg(uint32_t servertype, const uint32_t requestId, const std::string& strMsg)
{
	...
	{
		m_pHttpServerModule->ResponseMsg((NF_SERVER_TYPE)servertype, requestId, strMsg, NFWebStatus::WEB_OK, "OK");
	}
}
